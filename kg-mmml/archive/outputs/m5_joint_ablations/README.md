...
PyTorch joint model experiments testing consistency penalty (λ) variants:
...
M5 conclusion: Consistency penalty (λ > 0) did not improve performance. The simpler λ=0.0 model outperformed constrained variants.
...
3. Sklearn beats PyTorch for this task: Traditional ML simpler and better than deep learning
...
These experiments are retained for reproducibility and documentation only. Do not use these configurations for production or further research. Use the sklearn baseline instead.
