...
This chapter details the research design, data collection procedures, metric definitions, and experimental protocols used to evaluate the hybrid KG-ML architecture. The methodology operationalises the theoretical framework established in Chapter 2 into measurable decision gates and reproducible experimental procedures.

The domain for this research is **SEC Financial Documents**—specifically the 10-K (annual) and 10-Q (quarterly) filings that every public US company must submit. These documents contain thousands of "facts" tagged with XBRL (eXtensible Business Reporting Language), providing semi-structured data suitable for knowledge graph construction.
...
Pure graph queries are the opposite. They preserve meaning perfectly but struggle to scale. Complex SPARQL queries over millions of nodes cannot reliably meet 100ms service-level objectives.

This research attempts to resolve that tension through a **hybrid architecture** that uses a graph as a "semantic spine" for governance and structure, whilst relying on a vector index for the heavy lifting of retrieval. The goal is to determine whether this architecture can maintain the speed of vectors without losing the meaning of the graph.

### 3.1.2 Multi-Modal Definition: Structured Knowledge + Text

This work interprets "multi-modal machine learning" as the fusion of **structured knowledge** (knowledge graphs) with **unstructured text**, following Chen et al. (2024) and Zhang et al. (2019) who categorise KG-augmented systems as multi-modal when bridging symbolic (graph edges, typed entities) and sub-symbolic (continuous embeddings) representations.
...
2. **Unstructured modality**: Concept labels and textual descriptions processed via TF-IDF vectorisation

This contrasts with **perceptual multi-modality** (vision+language systems like CLIP) but demonstrates the same integration challenges: semantic loss during embedding, scalability trade-offs, and evaluation honesty. The hybrid architecture patterns validated here generalise to perceptual domains by substituting encoders (e.g., CLIP for TF-IDF) whilst preserving the graph semantic spine.

### 3.1.3 Hybrid Architecture Rationale

The literature review (Chapter 2, Section 2.4) identified three integration patterns: KG-as-features, joint objectives, and retrieval-time routing. This project implements and evaluates all three, with the hybrid architecture (retrieval-time routing) selected as the production configuration based on:
...
- **Specialisation principle**: Purpose-built systems (graph for structure, vector for similarity) outperform monolithic stores under mixed workloads
...
- **Hypothesis H3**: Approximate nearest neighbour (ANN) methods (Annoy, FAISS-HNSW) will achieve p99 < 150ms with 4,000× margin at target scale.
...
### 3.2.1 Data Source: SEC EDGAR CompanyFacts
...
**Script**: `src/data/normalise_facts.py`
...
3. **Unit standardisation**: Normalise "USD" vs "usd" vs "iso4217:USD" to canonical "USD"
...
### 3.2.4 Concept Space Profiling

**Analysis**: Before building the KG, concept frequency and CIK support were profiled to inform taxonomy generation.
...
This profile informed the **min_cik_support=3** threshold in frequency-based taxonomy rules (Section 3.3).
...
### 3.3.2 Knowledge Graph Snapshot Creation
...
2. **Edge generation**:
   ...
   - is_a: Load from combined taxonomy CSV (Section 3.3.3)
...
### 3.3.3 Taxonomy Generation: The Auto-Taxonomy Approach

Taxonomy construction became the critical bottleneck of this project. Initial attempts with manual curation produced only 27 edges, yielding Hierarchy Presence (HP) = 1.15%. This approach did not scale to the 4,508-concept corpus.

The solution: **auto-taxonomy generation** using pattern rules to automatically generate is-a relationships. This approach increased HP to 27.26%—a 2,370% improvement.

#### Manual Seed Taxonomy

**Source**: `datasets/sec_edgar/taxonomy/usgaap_min.csv`

**Content**: 27 curated is-a relationships representing balance sheet and income statement backbone structure.
...
**Application**: After combining manual + pattern + frequency + backbone edges, materialise all ancestor paths.
...
**Implementation note**: Closure is optional (flag `--with_closure`). For this project, closure was **enabled** to maximise HP.

**Effect**: Adds ~200 implicit edges (total edges after closure: 1,891).

### 3.3.4 Taxonomy Build Script
...
### 3.4.1 Text Features: TF-IDF Vectorisation
...
**Processing**:
1. **Tokenisation**: CamelCase splitting ("AccountsReceivableNetCurrent" → ["Accounts", "Receivable", "Net", "Current"])
...
**Rationale**: TF-IDF provides a **strong text-only baseline** (98.32% micro-F1, see Chapter 5). This is critical for validating that KG features add value beyond label co-occurrence.

### 3.4.2 Concept Features: Binary Indicators (KG-as-Features)
...
## 3.5 Model Training and Experimental Configurations

### 3.5.1 Baseline: Text-Only Classification
...
**Hyperparameters**:
- `solver='liblinear'`: Optimised for multi-label sparse data
...
- `random_state=42`: Fixed seed for reproducible weight initialisation
...
- **Stable**: No sensitivity to random initialisation (deterministic with fixed seed)
...
### 3.5.2 Joint: Text + Concept Features
...
### 3.5.4 Consistency Penalty Experiments (PyTorch Joint Model)
...
**Parent-support distribution**: For each filing, compute the proportion of observed child concepts that map to each parent category (using is_a edges). Penalises predictions that deviate from this distribution.
...
### 3.6.1 Semantic Retention Score (SRS)
...
#### Relation Type Fidelity (RTF)

**Definition**: Embedding-based metric measuring whether learnt embeddings preserve relation type distinctions (is_a vs measured_in vs for_period).

**Implementation**: Train TransE embeddings on knowledge graph; use probe classifier to predict relation type from (head, tail) embedding pairs.

**Computation**:
```python
# Train TransE on KG triples
model = TransE(n_entities, n_relations, embedding_dim=50)
model.fit(triples, epochs=100)

# Extract embeddings
entity_embeddings = model.entity_embeddings
relation_embeddings = model.relation_embeddings

# Train probe classifier: (h, t) → relation_type
X_probe = [concatenate(entity_embeddings[h], entity_embeddings[t]) for (h, r, t) in test_triples]
y_probe = [r for (h, r, t) in test_triples]
classifier = LogisticRegression()
classifier.fit(X_probe_train, y_probe_train)

# RTF = probe accuracy
RTF = classifier.score(X_probe_test, y_probe_test)
```

**Interpretation**: RTF=1.0 means embeddings perfectly distinguish relation types; RTF=0.33 (random guess for 3 types) means no preservation.

**Weight rationale (0.35)**: Highest weight because RTF directly measures whether embeddings retain semantic distinctions, the core research question.
...
**Interpretation**: Emphasises **rare class performance**. A system with perfect accuracy on common classes but zero recall on rare classes will have low macro-F1.

**Use case**: Demonstrates KG benefit for long-tail categories (e.g., specialised asset types).

### 3.6.3 Latency Benchmarking
...
3. **annoy**: Approximate nearest neighbours (trees=20, SVD-256)
...
## 3.7 Decision Gates and Thresholds
...
- RTF ≥ 0.85 (probe classifier accuracy; distinguishes relation types)
→ SRS = 0.25×0.25 + 0.20×0.95 + 0.20×0.99 + 0.35×0.85 = 0.7575 (target achieved if all component gates pass).

**Note on Deviation**: The Literature Review initially hypothesised a relative improvement of +25% over baseline. However, given the baseline SRS of ~0.67, a +25% increase would require SRS ≈ 0.84, which proved unrealistic given the conservative auto-taxonomy approach. The absolute threshold of 0.75 was adopted as a more robust and achievable quality gate.

### 3.7.2 +3pp Micro-F1 Improvement
...
**Potential bias**: US-GAAP taxonomy reflects US accounting standards. May not generalise to IFRS (international) or industry-specific taxonomies.

**Mitigation**: Scope clearly defined as "US SEC filings"; limitations discussed in Results chapter.

### 3.9.3 Transparency

- All code open-sourced (GitHub: repository link to be added)
- Data sources publicly accessible (SEC EDGAR API)
- Configs and outputs archived for replication

### 3.9.4 Dual-Use Considerations
...
## 3.10 Limitations and Scope Boundaries
...
- **Generalisation claim**: Hybrid architecture patterns extend to vision by substituting CLIP for TF-IDF

### 3.10.3 Domain

- **Focus**: Financial documents (SEC filings)
- **Generalisation**: KG integration principles (SRS metric, hybrid routing) are domain-agnostic; demonstrated on finance but applicable to manufacturing (defect ontologies), healthcare (medical KGs), e-commerce (product taxonomies)

### 3.10.4 Taxonomy Depth
...
- **Auto-taxonomy**: 2,370% HP uplift via conservative pattern rules and transitive closure
...
**Chapter 4 (Implementation) provides technical details of system architecture, feature pipelines, and deployment considerations. Chapter 5 (Results) presents decision gate outcomes, robustness tests, and error analysis.**
